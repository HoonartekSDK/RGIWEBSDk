(function (factory) {
    ...
    //}
     ...
     //kore customization starts Done : button and for None of the above 
    ...
    // 2new
    // $(document).on('click', '.fromOtherUsers.with-icon', function() {
    //     let sdcVal = Number(sessionStorage.getItem('sdc'));  // Get the value from sessionStorage, default to 0 if not set
    //     var checkboxes = document.querySelectorAll('.checkInput');  //
    //     const noneOfTheAboveValue = "None of the above";
...
// When the template is rendered, make sure to reset the states
    ...
    return function koreBotChat() {
        ...
        window.enableMicAutoOnOff = function enableMicAutoOnOff(){
            ...
            if (window.micAutoOnOff){
                ...
                document.getElementById("mic-auto-btn").style.backgroundColor = "white";
            }
        }
        /* Hoonartek Customization ends for mic*/
        var koreJquery;
        ...
        function updateOnlineStatus() {
            ...
            chatWindow.prototype.bindEvents = function () {
                ...
                _chatContainer.off('click', '.buttonTmplContentBox li,.listTmplContentChild .buyBtn,.viewMoreList .viewMore,.listItemPath,.quickReply,.carouselImageContent,.listRightContent,.checkboxBtn,.likeDislikeDiv,.buttonQuickReply, .done-button, .doneBtn').on('click', '.buttonTmplContentBox li,.listTmplContentChild .buyBtn, .viewMoreList .viewMore,.listItemPath,.quickReply,.carouselImageContent,.listRightContent,.checkboxBtn,.likeDislikeDiv,.buttonQuickReply, .done-button, .doneBtn', function (e) {
                    ...
                    // this code is for after done button action
                    ...
                    // hoonartek for checkbox template
            
                    console.log('before thattttttt in if of list done buuton')
            ...
            chatWindow.prototype.renderMessage = function (msgData) {
                ...
                if (msgData.type === "bot_response") {
                    ...
                    setTimeout(function () {
                        ...
                        /* Hoonartek Customization starts for mic */
                        if (window.micAutoOnOff && !chatInitialize.isTTSOn){
                            console.log("From @2374: ");
                            $('.notRecordingMicrophone').trigger('click');
                        };
                        /* Hoonartek Customization ends mic */                        
                    }, 500);
                }
                else {
                    waiting_for_message = false;
            ...
            chatWindow.prototype.getChatTemplate = function (tempType) {
                ...
                {{/if}} \
                {{if isSpeechEnabled}}\
                <div class="sdkFooterIcon microphoneBtn" mic-auto> \
                    <div class="sdkFooterIcon mic-auto-on-off">\
                    <button id="mic-auto-btn" onclick="window.enableMicAutoOnOff();"> \
                            Auto Mic \
                    </button> \
                </div>\
                <div class="sdkFooterIcon microphoneBtn"> \
                    <button id="notRecordingMicBtn" class="notRecordingMicrophone" title="Microphone On"> \
     ...
     var iframe = '<script id="chat_message_tmpl" type="text/x-jquery-tmpl"> \
        ...
        // hoonartek customization start to  disable the button
